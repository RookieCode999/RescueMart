...
<?php
	...
	$stmt0 = $conn->query("SELECT shopName,shopCategory FROM rescuemart.shop where shopId = $shopid;");
...
<?php
	$image="";
		$dir = 'uploads'; 
		$stmti = $conn->query("SELECT img1 FROM rescuemart.shopimage where shopId = $shopid;");
...
<?php
	$image="";
	$dir = 'uploads'; 
	$stmt = $conn->query("SELECT productName,pid,img1,img2,img3,price FROM rescuemart.product where shopId = $shopid;");
...
?>
	<div   class="product" style=" background-image: url('<?php echo $image; ?>'); background-size: 100% 100%; background-repeat: no-repeat;">
		...
		<form action='productOper.php' method='POST'
			...
			<input class="addstyle nonClick" style="margin-top: 196px;" type='submit' name='' value="Price : <?php echo $row['price']; ?>" />
			<br>
