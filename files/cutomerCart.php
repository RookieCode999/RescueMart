...
<?php
	...
	session_start();
	$cid = $_SESSION['NavCid'];
	$stmt0 = $conn->query("SELECT pid FROM rescuemart.cart where cid = $cid;");
	$row0=$stmt0->fetch(PDO::FETCH_ASSOC);
	$stmt1 = $conn->query("SELECT name FROM rescuemart.customer where cid = $cid;");
...
<body>
  ...
  <div class="site-pusher">
	...
	<?php 
		$stmtit = $conn->query("SELECT count(*) as totalItems FROM rescuemart.cart where cid = $cid;");
		$stmtprice = $conn->query("SELECT sum(price) as Total FROM rescuemart.product P, rescuemart.cart A WHERE P.pid = A.pid AND A.cid= $cid;");
...
<?php
	$image="";
	$dir = 'uploads'; 
	$stmt = $conn->query("SELECT pid FROM rescuemart.cart where cid = $cid;");
	while ($row=$stmt->fetch(PDO::FETCH_ASSOC)){
		$pid = $row['pid'];
		$stmt1 = $conn->query("SELECT productName,pid,img1,img2,img3,price FROM rescuemart.product where pid = $pid;");
		$row1=$stmt1->fetch(PDO::FETCH_ASSOC);
