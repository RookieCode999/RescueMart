...
<?php
	...
	$stmt0 = $conn->query("SELECT serName,shopId,category,baseCharge,img1,img2,info FROM rescuemart.service where serId = $serId;");

	$row0=$stmt0->fetch(PDO::FETCH_ASSOC);
	$shopId = $row0['shopId'];
	$stmtshop = $conn->query("SELECT shopName,shopContact,shopEmail FROM rescuemart.shop where shopId = $shopId;");
