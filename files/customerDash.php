...
<?php
	...
	session_start();
	$cid = $_SESSION['NavCid'];
	$stmt0 = $conn->query("SELECT name FROM rescuemart.customer where cid = $cid;");
    ...
    <header class="header">
      ...
      <nav class="menu">
        ...
        <div class="formNav">
			...
			<input class="simpleButton" type='submit' name='Psubmit' value='RescueMart' />
...
<?php
	$image="";
	$dir = 'uploads'; 
	$stmt = $conn->query("SELECT productName,pid,img1,img2,img3,price FROM rescuemart.product;");
...
?>
	<div style=" background-image: url('<?php echo $image; ?>'); background-size: 100% 100%; background-repeat: no-repeat;"  class="product">
		...
		<form action='productOperCustomer.php' method='POST'
			...
			<input class="addstyle nonClick" style="margin-top: 236px;" type='submit' name='' value="Price : <?php echo $row['price']; ?>" />
