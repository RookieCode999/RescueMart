...
<?php
	...
	$stmt0 = $conn->query("SELECT balance,name FROM rescuemart.seller where sid = $ownerSid;");
...
<?php
	$image="";
	$dir = 'uploads'; 
	$stmt = $conn->query("SELECT shopName,shopId FROM rescuemart.shop where ownerSid = $ownerSid;");
	while ($row=$stmt->fetch(PDO::FETCH_ASSOC)){
		$shopId = $row['shopId'];
		$stmt1 = $conn->query("SELECT img1 FROM rescuemart.shopimage where shopId = $shopId;");
