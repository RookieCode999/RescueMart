<?php
    ...
    // Use isset to check if the key exists in $_SESSION
    $cid = isset($_SESSION['NavCid']) ? $_SESSION['NavCid'] : '';

    // Use the same approach for shopId
    $shopId = isset($_POST['shopId']) ? $_POST['shopId'] : '-9999';
    ...
    $stmt0 = $conn->prepare("SELECT name FROM rescuemart.customer where cid = :cid");
    $stmt0->bindParam(':cid', $cid);
    $stmt0->execute();
    $row0 = $stmt0->fetch(PDO::FETCH_ASSOC);

    $stmtshop = $conn->prepare("SELECT shopName, shopCategory, shopEmail, shopContact, shopInfo FROM rescuemart.shop where shopId = :shopId");
    $stmtshop->bindParam(':shopId', $shopId);
    $stmtshop->execute();
...
<?php
	$image="";
		$dir = 'uploads'; 
		// Use prepared statement for shopimage query
		$stmti = $conn->prepare("SELECT img1 FROM rescuemart.shopimage WHERE shopId = :shopId");
		$stmti->bindParam(':shopId', $shopId);
		$stmti->execute();
...
<?php
	$image="";
	$dir = 'uploads'; 
	// Use prepared statement for product query
	$stmt = $conn->prepare("SELECT productName, pid, img1, img2, img3, price FROM rescuemart.product WHERE shopId = :shopId");
	...
	while ($row=$stmt->fetch(PDO::FETCH_ASSOC)){
		$productId= $row['pid'];
		$image = $dir.'/'.$row['img1'];
		// $stmt2 = $conn->query("SELECT img1 FROM rescuemart.shopimage where shopId = $shopid;");
